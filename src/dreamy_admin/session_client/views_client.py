...
from .forms import ClientForm, ClientFormShort, SearchClientForm
...
def client_search_view(request):
    ...
    if request.method == "POST":
        ...
        if form_partial.is_valid():
            ...
            sessions = (
                SessionModel.objects.filter(client__user=request.user)
                .annotate(
                    search=vector,
                    rank=SearchRank(vector, search_query),
                    headline=SearchHeadline("keywords", search_query),
                )
                .filter(search=raw_query)
                .order_by("-rank")
            )
            if client:
...
def add_client_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            context = {"client": client}
            return ok_response_render(
                request, partial_template, context, "me", ("CloseModal", ".modal"), "beforebegin"
            )
