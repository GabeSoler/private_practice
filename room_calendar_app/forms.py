...
class LinkTenantForm(forms.Form):
    room_id = forms.UUIDField(required=True, label='Room Code')


class WeekCalendarForm(forms.Form):
    """calendar switch form"""
    date_reference = forms.DateField(widget=forms.DateInput(attrs={"type": "date", }), required=True,
...
class RoomSwitchForm(forms.Form):
    calendar = forms.ModelChoiceField(help_text="Select a room to see its blocks", required=False,
                                      queryset=RoomCalendarModel.objects.all())
